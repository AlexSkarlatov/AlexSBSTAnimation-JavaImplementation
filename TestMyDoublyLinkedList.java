...
public static void main(String[] args) {
 ...
 list.add("Alabama");
	...
	//ask a question about why i cannot get this..???????TODO
	//i think the problem is with my getNode iterator loop
...
//test these guys right here
	...
	/////this si where the error happened so
	//it looks like lastIdexOF(is the problem)
	//apparently it thinks the answer is 4 for some reason
	...
	if (list.getLast() != "California")
		throw new RuntimeException();
	try {
		list.get(-1);//PROBLEM HERE THROW EXCEPTION INDEX OUTOFBOUNDSEXCEPTION
	...
	listIter.add("Kentucky");//problem with my add method
	listIter.next();
	listIter.set("Ohio");

	// [Indiana, Kentucky, Ohio]
	if (list.size() != 3)//so thereis something wrong with the size variable when did the size go wrong
		throw new RuntimeException();//error right here//error solved i just created a method that returns the value of the size data field
	if (list.getFirst() != "Indiana" || list.getLast() != "Ohio")//im guessing my list is wron
		throw new RuntimeException();
